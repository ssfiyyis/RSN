...
class Config:
    ...
    # -------- Training Config -------- #
    SOLVER = edict()
    SOLVER.BASE_LR = 7.5e-4       # according to x1.5 batch size
    SOLVER.CHECKPOINT_PERIOD = 1600
    SOLVER.GAMMA = 0.5
    SOLVER.IMS_PER_GPU = 96       # according changing number of GPUs 8 -> 4, x1.5 batch size
    SOLVER.MAX_ITER = 28800
    SOLVER.MOMENTUM = 0.9
    SOLVER.OPTIMIZER = 'Adam'
    SOLVER.WARMUP_FACTOR = 0.1
    SOLVER.WARMUP_ITERS = 1600    # according to x1.5 batch size
